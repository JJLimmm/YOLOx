...
class AnnotationTransform(object):
    ...
    def __init__(
        ...
        self.keep_fake = keep_fake
        self.keep_occluded = keep_occluded
    ...
    def __call__(self, target):
        ...
        for obj in target.iter("object"):
            ...
            def check_property(prop: str) -> bool:
                prop = obj.find(prop)
                if prop is not None:
                    try:
                        check = int(prop.text) > 0
                        return check
                    except:
                        return False
                else:
            ...
            if not self.keep_difficult and check_property("difficult"):
                continue
            if not self.keep_fake and check_property("fake"):
                continue
            if not self.keep_occluded and check_property("occluded"):
                continue
            if not self.keep_truncated and check_property("truncated"):
                continue
            ...
            try:
                ...
                continue  # skip labels not in defined classes
